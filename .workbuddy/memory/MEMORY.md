...
- 体检 `.workbuddy/tmp/linkcheck.py`（跑 **py3.13.12 managed**）：112 路由/46 内链/死链 0/缺图 0。溢出审计 `overflow-all.cjs`(playwright-core+Edge，33 页×9 宽=297 项，需先起 8088 静态服务)。
...
- **⚠️ `getCollection()` 必须用连字符名**：config 里 key 写 camelCase（`caseStudies`/`blogEs`），但 `getCollection('caseStudies')`/`('blogEs')` **取不到数据**（不报错，静默返回空 → 动态路由直接不生成页面）。全站统一用 `'case-studies'` / `'case-studies-es'` / `'blog'` / `'blog-es'`。踩过一次：西语案例详情页因此从 33 页掉到 29 页且无报错。
...
- **顶栏渐变（`Header.astro` + `global.css`）**：`--hdr-wp`(白层满格终点≈logo 右缘+30px)/`--hdr-wf`(收尾长度)/`--hdr-kz`(墨绿铺满≈首个导航项)/`--hdr-w-a .64`/`--hdr-k-a .72`。正解=白层满格到 `--hdr-wp`，再用 `f(t)=1-smoothstep(t)^0.6`（首尾斜率 0）收尾。**两个反面教材**：①实心白平台+线性过渡→平台尽头有硬拐点；②白层从 x=0 就衰减→宽屏 logo 掉进灰底。改完必看 1920/1440/1000/390 四档。
...
- **西语已同步**（2026-09-14）：`es/index|services|about` 按英文新结构重写；`es/about` 已删虚构成员（Amara/Carlos），只留 Xizeng。西语页源码零中文。
- **西语正文架构**：英文正文 `src/content/blog|case-studies/`，西语正文 `src/content/blog-es|case-studies-es/<同 slug>.md`。解析顺序「`-es` 全文 → 英文 md 的 `*Es` → 英文」，没翻译自动回退并提示。加西语=丢同名 md 进 `-es`。`es/blog/[slug].astro` 是西语博客详情页。
